...
* :code:`lain job` 会启动一个 Kubernetes Job 容器, 来执行你给定的命令. 如果未给定命令, 则进入容器内, 打开 shell 进行交互操作. 在 lain app 内执行时, 可以用 :code:`--deploy` 明确指定参考哪个 deployment 的 pod template 来创建 job; 如果不指定, 默认取 :code:`values.deployments` 里定义的第一个 deployment.
...
如果你想显式指定参考模板, 最常见的写法如下:

.. code-block:: bash

    lain job --deploy worker -- bash

这里的参考模板主要会影响 job container 使用的 image、env、volumeMounts、volumes 和 hostAliases。
