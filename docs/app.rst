...
* :code:`lain job x` 可以进入已经存在的 Job 容器, 用法类似 :code:`lain x`. 默认会挑选当前应用最近一个正在运行的 Job Pod; 如果第一个参数正好是运行中的 job 名, 就优先进入该 Job, 否则就把这些参数当作容器内要执行的命令. 当命令自身带选项时, 也建议用 :code:`--` 隔开.
